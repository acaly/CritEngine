...
public class ObstacleFactory {
	 ...
	 * @param template
	 * @param weight
	 *            The weight used in weighted random. Don't need to be
	 *            normalized. (That is, the sum of weight in each template may
	 *            not be 1.0.)
	 */
	...
	private double generateRange(double fromX, double toX) {
		...
		for (i = fromX; i > toX; i -= getSpacing(i)) {
			getTemplate(weightedRandom.next(rand.nextDouble())).generate(i,
					rand.nextDouble() * GEN_HEIGHT + GEN_MIN_Y);
		}
		return i;
	}

	public void generateTo(double toX) {
		if (toX - GEN_AHEAD > lastGenTo)
			return;
