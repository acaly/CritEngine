...
public class CEPhysicEngine implements IEntityProcessor {
	...
	/* 执行一次光线追踪，并返回结果 */
	public static RayTraceResult rayTrace(Scene scene, Vector2d vec0, Vector2d vec1, IEntityFilter filter, Entity... exclusions) {
		double x0 = vec0.x,
				y0 = vec0.y,
				x1 = vec1.x,
				y1 = vec1.y;
	...
	private static RayTraceResult solve(Vector2d beg, Vector2d end, Entity ent) {
		...
		if(beg.equals(end)) return NULL_RESULT;
		
		if(dx == 0) { //Vertical line
			if(dy > 0) {
				return inrange(beg.y, rect.getMinY(), end.y) ? 
					new RayTraceResult(EnumEdgeSide.BOTTOM, ent, new Vector2d(beg.x, rect.getMinY()))
					: NULL_RESULT;
			} else {
				return inrange(end.y, rect.getMaxY(), beg.y) ? 
					new RayTraceResult(EnumEdgeSide.TOP, ent, new Vector2d(beg.x, rect.getMaxY())) 
					: NULL_RESULT;
			}
		}
		if(dy == 0) { //Horizonal line
			...
			if(dx > 0) {
				...
				return inrange(beg.x, rect.getMinX(), end.x) ? 
						new RayTraceResult(EnumEdgeSide.LEFT, ent, new Vector2d(rect.getMinX() - 0.001, beg.y))
						: NULL_RESULT;
			} else {
				//System.out.println("JDG2");
				return inrange(end.x, rect.getMaxX(), beg.x) ?
						new RayTraceResult(EnumEdgeSide.RIGHT, ent, new Vector2d(rect.getMaxX(), beg.y)) :
						NULL_RESULT;
			}
		}
		
		double k = dy / dx,
				b = (end.x * beg.y - beg.x * end.y) / dx;
		double temp = 0;
		...
		if(dy > 0) { //BOTTOM?
			...
			if(inrange(a0, temp, a1))
				return new RayTraceResult(EnumEdgeSide.BOTTOM, ent, new Vector2d(temp, rect.getMinY()));
		} else { //TOP?
			temp = (rect.getMaxY() - b) / k;
			if(inrange(a0, temp, a1))
				return new RayTraceResult(EnumEdgeSide.TOP, ent, new Vector2d(temp, rect.getMaxY()));
		}
		...
		if(dx > 0) { //LEFT?
			...
			if(inrange(a0, temp, a1))
				return new RayTraceResult(EnumEdgeSide.LEFT, ent, new Vector2d(rect.getMinX(), temp));
		} else { //RIGHT?
			temp = k * rect.getMaxX() + b;
			if(inrange(a0, temp, a1))
				return new RayTraceResult(EnumEdgeSide.RIGHT, ent, new Vector2d(rect.getMaxX(), temp));
		}
